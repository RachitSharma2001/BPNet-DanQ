...
# They create BPNet as a type of SeqModel
# SeqModel is a wrapper class around an already defined Keras Model
'''
General Structure of this class:
init -> initializes a seqmodel type, the tasks to perform on, and the input sequences file
getseq -> prepares input by reading input sequences, creates one hot vector
predict, predict_all, predict_regions -> methods written for prediction
    predict_regions -> calls predict_all, then for output it adds label of region
    predict_all -> goes through all DNA sequences in given input file, for each sequence calls predict
    predict -> goes through sequence, predicts where tfs will bind
contrib_score_all -> compute contribution scores over the input
...
General theme of this class:
    ...
    predictions on regions of the DNA binding of the 4 or so different tasks(tfs).
...
class BPNetSeqModel:
    ...
    # Here we initalize our own seq model, and then add some other parameters
    # such as tasks and a fasta file
    # tasks -
    ...
    # ------------ Seems to be methods that extract inputs to put into model ----------------
    @classmethod
    ...
    # Predicts on a bunch of tfs
    # preds is a dict -> what does this function do to pred? -> maybe it
    # returns a new dict, where in it is preds[each task]? so we loop through
    # each task and we call preds[task] and put it into dict?
    def predict(self, seq, batch_size=512):
        ...
        """
        # self.seqmodel is a specific instance of the generic seqmodel class
        # so we just call predict on it -> thats why we don't need to pass in model weights
        # preds is a dictionary -> so the output of the bpnet from looking at the paper should be
        # signals over the sequence -> how does preds represent this?
        #
    ...
    def contrib_score_all(self, seq, method='deeplift', aggregate_strand=True, batch_size=512,
        ...
        '''
        They seem to use the generic "seqmodel" class that they made on their own
        What does contrib_score_all() do with the parameters? To calculate contributation scores, wouldn't
        contrib_score_all need to know the output for this input seq? -> Answer: I believe because we are
        only running motif-extracting methods AFTER we have fully trained the model, we only compute
        contribuation scores after fully training model, so what contrib_score_all does is take the input sequence,
        load the trained models weights, run seq through model to get output, and then computes the contrib scores

        Also: contrib_scores is a dictionary, returns contrib score for given task/profile
        '''
    ...
    # Takes the file that has raw sequences and creates a one hot encoded vector from it
    ...
    # very interesting method -> calls predict_all, which then calls predict
    # What does this return -> it just returns the predictions made by predict_all
    # but it adds a label indicating what the regions are in preds
    # This is important when doing tf modisco and other methods -> need to identify which regions
    # tfs are bounding to the most, so having a region label helps
